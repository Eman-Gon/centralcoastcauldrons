...
def get_wholesale_purchase_plan(wholesale_catalog: list[Barrel]):
    ...
    with db.engine.begin() as connection:
        ...
        num_red_potion = connection.execute(sqlalchemy.text("SELECT num_red_potions FROM global_inventory")).scalar_one()
        num_blue_potion = connection.execute(sqlalchemy.text("SELECT num_blue_potions FROM global_inventory")).scalar_one()
        num_dark_potion = connection.execute(sqlalchemy.text("SELECT num_dark_potions FROM global_inventory")).scalar_one()
        gold = connection.execute(sqlalchemy.text("SELECT gold FROM global_inventory")).scalar_one()
        ...
        for barrel in wholesale_catalog:
            ...
            elif barrel.sku == 'SMALL_RED_BARREL' and num_red_potion < 10 and gold >= barrel.price:
                barrel_plan.append({"sku": barrel.sku, "quantity": 1})
            elif barrel.sku == 'SMALL_BLUE_BARREL' and num_blue_potion < 10 and gold >= barrel.price:
                barrel_plan.append({"sku": barrel.sku, "quantity": 1})
            elif barrel.sku == 'SMALL_DARK_BARREL' and num_dark_potion < 10 and gold >= barrel.price:
                barrel_plan.append({"sku": barrel.sku, "quantity": 1})

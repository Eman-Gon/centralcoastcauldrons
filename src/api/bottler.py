...
def get_bottle_plan():
    ...
    with db.engine.begin() as connection:
        ...
        for potion in potions:
            ...
            potion_type = [num_red_potions * 50, num_green_potions * 50, num_blue_potions * 50, num_dark_potions * 50]

            available_quantities = [num_red_ml, num_green_ml, num_blue_ml, num_dark_ml]
            
            max_quantities = []
            for i in range(4):
                if potion_type[i] > 0 and available_quantities[i] >= potion_type[i]: max_quantities.append(available_quantities[i] // potion_type[i])
                else:
